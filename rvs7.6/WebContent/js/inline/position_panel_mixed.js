...
var servicePathMixed = "position_panel_mixed.do";
		...
		buttons : {
			"确定":function(){
				if ($("#breakForm").valid()) {
					...
					var postUrl = servicePath + '?method=dobreak';
					if (!$("#pauseo_material_id").val()) {
						postUrl = servicePathMixed + '?method=dobreak';
						data.serial_no = $("#material_details td:eq(5)").text();
					}
...
//						comments : $("#pauseo_edit_comments").val(),
					...
					var postUrl = servicePath + '?method=dopause';
					if (!$("#pauseo_material_id").val()) {
						postUrl = servicePathMixed + '?method=dopause';
					}
					// Ajax提交
					$.ajax({
						beforeSend : ajaxRequestType,
...
var endPause = function() {
	...
	var postUrl = servicePath + '?method=doendpause';
	if (!$("#pauseo_material_id").val()) {
		postUrl = servicePathMixed + '?method=doendpause';
		data.serial_no = $("#material_details td:eq(5)").text();
	} else {
		data.material_id = $("#pauseo_material_id").val();
		data.workstauts = $("#hidden_workstauts").val();
	}
...
var treatPause = function(resInfo) {
	...
	$("#waitings").removeClass("waitForStart");
	$(".opd_re_comment").show();
	$("#pausebutton").hide();
...
var treatStart = function(resInfo) {
	...
	if (resInfo.mform) {
		var wKey = resInfo.mform.material_id;
		if (!wKey) {
			wKey = resInfo.mform.model_id + "_" + resInfo.mform.serial_no;
		}
		$("#w_" + wKey).hide("drop", {direction: 'right'}, function() {
			var jthis = $(this);
			...
			if (resInfo.jsinitInfect) {
				setTimeout(function(){
					...
					$.ajax({
						beforeSend : ajaxRequestType,
						async : true,
						url : 'position_panel.do?method=jsinitInfect',
						cache : false,
						data : null,
						type : "post",
						dataType : "json",
...
//			}
			...
			$("#waitings").addClass("waitForStart");
...
var getJustWorkingFingers = function(material_id, model_id, serial_no) {

	var postUrl = servicePath + '?method=doPointOut';
	if (material_id == null) {
		postUrl = servicePathMixed + '?method=doPointOut';
	}
...
var doInit=function(){
	...
	$.ajax({
		beforeSend : ajaxRequestType,
		async : false,
		url : servicePathMixed + '?method=jsinit',
		cache : false,
		data : {},
		type : "post",
		dataType : "json",
...
var reportAndBreak = function() {
	...
	$.ajax({
		beforeSend : ajaxRequestType,
		async : false,
		url : 'position_panel.do?method=doCallLeaderOfPartialMismatch',
		cache : false,
		data : postData,
		type : "post",
		dataType : "json",
...
var commitPartialConfirm = function($target, finish) {
	...
	$.ajax({
		beforeSend : ajaxRequestType,
		async : false,
		url : 'position_panel.do?method=doPartialUse',
		cache : false,
		data : postData,
		type : "post",
		dataType : "json",
...
var doFinish=function(evt, hr){
	...
	if (!$("#pauseo_material_id").val()) {
		postUrl = servicePathMixed + '?method=dofinish';
		data.serial_no = $("#material_details td:eq(5)").text();
		if ($(".instockCode").length > 0) {
			data.stock_code = $(".instockCode").text();
			if (!data.stock_code) {
				errorPop("请指定组件入库库位后，再完成本工位作业。");
				return;
			}
		}
	}
...
var getWaitingHtml = function(waitings, other) {
	...
	for (var iwaiting = 0; iwaiting < waitings.length; iwaiting++) {
		...
		if (waiting.fix_type == "8") {
			waiting_html += getWaitingHtmlComp(waiting);
		} else {
...
var getWaitingHtmlComp = function(waiting) {
	var waiting_html = "";

	waiting_html += '<div class="waiting tube' + (waiting.imbalance ? "" : " other_px") + '"' 
					+ ' model_id="' + waiting.model_id + '" model_name="' + waiting.model_name + '" serial_no="' + waiting.serial_no + '"' +
					' id="w_' + waiting.model_id + "_" + waiting.serial_no + '">' +
						'<div class="tube-liquid tube-comp">' +
							(waiting.sorc_no == null ? "" : waiting.sorc_no + ' | ') + waiting.model_name + ' | ' + waiting.serial_no
							+ (waiting.overtime == 2 ? "<div class='material_flags'><div class='over_time'>超</div></div>" : "")
							+ getDryingTime(waiting.drying_process) +   
						'</div>' +
					 (waiting.imbalance ? '<div class="click_start"><input type="button" value="》开始"></div>' : '') +
					'</div>'
...
var showWaitings = function(waitings){
	var waitingsOtherPx = [];
	var $waiting_html = $(getWaitingHtml(waitings, waitingsOtherPx));
...
var getWaitings = function() {
	$.ajax({
		data: null,
		url : servicePathMixed + "?method=refreshWaitings",
		type : "post",
		cache : false,
		dataType : "json",
		success : ajaxSuccessCheck,
...
var pxChange = function() {
	...
	$.ajax({
		...
		url : "position_panel.do?method=pxChange",
		type : "post",
		cache : false,
		dataType : "json",
		success : ajaxSuccessCheck,
...
var foundryChange = function(){
	...
	if (foundry_start_time ==  "") {
		...
		$.ajax({
			beforeSend : ajaxRequestType,
			async : true,
			url : 'position_panel.do?method=doFoundryChange',
			cache : false,
