...
var showContentDetail = function(){
	...
	content_dialog.load("widgets/manage/interface_data_edit.jsp" , function(responseText, textStatus, XMLHttpRequest) {
		...
		if (kind == "recept") {
			...
			$("#body-quotation_part").hide();
		}else if(kind=="quotation_part"){
			$("#body-approve").hide();
			$("#body-recept").hide();
			$("#body-part-order").hide();
			$("#body-quotation_part").show();
		}
...
var setValue = function(detailForm){
	...
	if(kind=="approve"){//投线
		...
		$("#recept_edit_failure_descrip").val(content.FailureDescrip);
		if (content.AnimalExperiment && content.AnimalExperiment.length) {
			$("#animal_experiment_yes").attr("checked","checked").trigger("change");
		} else {
		...
		$("#quotation_part_edit_omr_notifi_no").text(content.OMRNotifiNo);
		$("#quotation_part_edit_osh_rank").val(content.OSHRank);

		var itemsContent="";
		
		$.each(content.Items,function(index,item){
			itemsContent +='<tr class="two" index="'+ index +'">'
					+'<td class="ui-state-default td-title">BOM CODE '+ (index+1) +'</td>'
					+'<td class="td-content"><input type="text" class="ui-widget-content" name="BomCode" value="'+ item.BomCode +'"></td>'
				+'</tr>'
				+'<tr class="two" index="'+ index +'">'
					+'<td class="ui-state-default td-title">零件物料号</td>'
					+'<td class="td-content"><input type="text" class="ui-widget-content" name="PartsMaterialNo" value="'+ item.PartsMaterialNo +'"></td>'
				+'</tr>'
				+'<tr class="two" index="'+ index +'">'
					+'<td class="ui-state-default td-title">指示数量</td>'
					+'<td class="td-content"><input type="text" class="ui-widget-content" name="InstructQty" value="'+ item.InstructQty +'"></td>'
				+'</tr>'
				+'<tr class="two" index="'+ index +'">'
					+'<td class="ui-state-default td-title">Rank</td>'
					+'<td class="td-content"><input type="text" class="ui-widget-content" name="Rank" value="'+ (item.Rank || "") +'"></td>'
				+'</tr>'
				+'<tr class="two" index="'+ index +'">'
					+'<td class="ui-state-default td-title">Ship</td>'
					+'<td class="td-content"><input type="text" class="ui-widget-content" name="Ship" value="'+ (item.Ship || "") +'"></td>'
				+'</tr>'
				+'<tr class="two" index="'+ index +'">'
					+'<td class="ui-state-default td-title">Sorc Rank</td>'
					+'<td class="td-content"><input type="text" class="ui-widget-content" name="SorcRank" value="'+ (item.SorcRank || "") +'"></td>'
				+'</tr>';
		});

		$("#body-quotation_part table tbody").append(itemsContent);
	}
};
...
var comfirm_f = function(){
	...
	if(kind=="approve"){
		...
		data.edit_omr_notifi_no = $("#quotation_part_edit_omr_notifi_no").text();
		data.edit_osh_rank = $("#quotation_part_edit_osh_rank").val();

		$("#body-quotation_part table tr.two").each(function(idx,ele){
			var $tr = $(ele);
			var index = $tr.attr("index");
			...
			data["part_pre."+ column +"["+  index +"]"] = value;
		});
	}
	$.ajax({
		beforeSend : ajaxRequestType,
